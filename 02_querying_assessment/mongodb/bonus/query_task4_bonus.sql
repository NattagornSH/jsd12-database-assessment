...
--
-- Your thinking:
-- โจทย์ถามหายอดรวม revenue ทั้งหมดจากทุก order เป็นตัวเลขเดียว
-- ใช้ SELECT และหาผลรวมด้วย SUM(orders.total_price) เพื่อรวมทุก order แล้วตั้งชื่อด้วย AS
-- ดึงข้อมูลจาก table orders

SELECT SUM(orders.total_price) AS "Total revenue"
FROM orders;
